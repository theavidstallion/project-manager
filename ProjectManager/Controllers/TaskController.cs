    ...
    public class TaskController : ControllerBase
    {
        ...
        public async Task<IActionResult> DeleteTask(int id)
        {
            ...
            else if (User.IsInRole("Manager"))
            {
                ...
                {
                    return Forbid("Managers can only delete tasks they manage/created.");
                }
            }
            else
            {
        ...
        [Authorize(Roles = "Admin, Manager")] // Only Admins and Managers can update core details
        ...
        public async Task<IActionResult> UpdateTask(int id, [FromBody] UpdateTaskDto taskModel)
        {
            ...
            if (User.IsInRole("Member"))
            {
                return Forbid("Members are not authorized to update tasks.");
            }
            ...
            if (User.IsInRole("Manager") && !User.IsInRole("Admin"))
            {
                ...
                {
                    return Forbid("Managers can only update tasks within projects they manage.");
                }
            }

            // Note: Member role is excluded entirely from this endpoint based on your request.
        ...
        public async Task<IActionResult> ChangeTaskStatus(int id, [FromBody] ChangeTaskStatusDto statusDto)
        {
            ...
            if (User.IsInRole("Member"))
            {
                ...
                {
                    return Forbid("Members can only change status of tasks assigned to them.");
                }
            }
        ...
        public async Task<IActionResult> AddTagsToTask(int id, [FromBody] AddTagsDto tagsDto)
        {
            ...
            if (User.IsInRole("Member"))
            {
                if (task.AssignedUserId != currentUserId)
                {
                    return Forbid("Members can only add tags to tasks assigned to them.");
                }
            }
            else if (User.IsInRole("Admin") || User.IsInRole("Manager"))
            {
